...
public class Test2Btn : MonoBehaviour, InteractionListenerInterface
{
    ...
    void Update () {
        if (interactionManager != null && interactionManager.IsInteractionInited())
        {
            ...
            {
                case 1:
                    skipStay = skipButton.GetComponent<ButtonScript>().stay;
                    ...
                    else
                    {
                        ...
                        if (selectedObject == skipButton && isReleased && skipStay == true)
                        {
                            this.GetComponent<AudioSource>().enabled = true;
                            this.GetComponent<btnAudioControll>().enabled = true;
                            doButton.GetComponent<skipTu>().enabled = true;
                            falseButton();
                            selectedObject = null;
                            isReleased = false;
                        }
                ...
                case 2:
                    ...
                    else
                    {
                        ...
                        if (selectedObject == okButton && isReleased && okStay == true)
                        {
                            this.GetComponent<AudioSource>().enabled = true;
                            this.GetComponent<btnAudioControll>().enabled = true;
                            doButton.GetComponent<goBack>().enabled = true;
                            falseButton();
                            selectedObject = null;
                            isReleased = false;
                        }
