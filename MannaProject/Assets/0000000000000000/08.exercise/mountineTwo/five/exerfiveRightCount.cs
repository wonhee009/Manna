...
public class exerfiveRightCount : MonoBehaviour {
    ...
    private void Awake()
    {
        level = GameObject.Find("userInfo").GetComponent<UserInfo>().test2_2level;
    }
    ...
    void Update () {
        if (start == true)
        {
            if (start2 == false)
            {
                if (count == 3)
                {
                    ges.GetComponent<twoExerGes>().mode = 3;
                    start2 = true;
                }
                else
                {
